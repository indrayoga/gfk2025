...
class Laporanapt extends Rumahsakit {
	...
	public function penerimaanapotek(){
		...
		$data=array('sumberdana'=>$this->mlaporanapt->sumberdana(),
	...
	public function penjualanobatapotek(){
		...
		$data=array('items'=>$this->mlaporanapt->ambilIsiPenjualan($periodeawal,$periodeakhir,$kd_unit_apt),
	...
	public function penjualanapotek(){
		...
		$data=array('items'=>$this->mlaporanapt->getAllPenjualanApotek($periodeawal,$periodeakhir,$shiftapt,$is_lunas,$resep),					
					'periodeawal'=>$periodeawal,
					'periodeakhir'=>$periodeakhir,
					'shiftapt'=>$shiftapt,
					'is_lunas'=>$is_lunas,
	...
	public function distribusiapotek(){
		...
		$data=array('items'=>$this->mlaporanapt->getAllDistribusiApotek($periodeawal,$periodeakhir,$kd_unit_asal),
					'unitapotek'=>$this->mlaporanapt->ambilData('apt_unit'),
					'periodeawal'=>$periodeawal,
					'periodeakhir'=>$periodeakhir,
	...
	public function kartustok(){
		...
		if($this->input->post('tgl_expire')!=''){
			$tgl_expire=$this->input->post('tgl_expire');
		}
		if($this->input->post('harga_pokok')!=''){
		...
		$datafooter=array('jsfile'=>$jsfooter);
		$data=array('sumberdana'=>$this->mlaporanapt->sumberdana(),
					'items'=>$this->mlaporanapt->getKartuStokBatch($kd_obat,$kd_unit_apt,$kd_pabrik,$tgl_expire,$harga_pokok,$batch),
					'nama_obat'=>$nama_obat,
					'kd_obat'=>$kd_obat,
					'kd_unit_apt'=>$kd_unit_apt,
            		'kd_pabrik'=>$kd_pabrik,
	...
	public function ambilobatbynamabatch(){
		...
		$this->datatables->select('apt_stok_unit.kd_obat, replace(apt_obat.nama_obat,"\'","") as nama_obat,apt_pabrik.nama_pabrik,apt_stok_unit.batch,apt_stok_unit.tgl_expire,apt_stok_unit.harga_pokok, "Pilihan" as pilihan,apt_stok_unit.kd_unit_apt,apt_stok_unit.kd_milik,apt_stok_unit.kd_pabrik',false);
		$this->datatables->join('apt_obat','apt_stok_unit.kd_obat=apt_obat.kd_obat');
		$this->datatables->join('apt_pabrik','apt_stok_unit.kd_pabrik=apt_pabrik.kd_pabrik');
		$this->datatables->from("apt_stok_unit");
		$this->datatables->edit_column('pilihan', '<a class="btn" onclick=\'pilihobat("$1","$2","$3","$4","$5","$6","$7","$8")\'>Pilih</a>','apt_stok_unit.kd_obat, nama_obat,apt_stok_unit.kd_unit_apt,apt_stok_unit.kd_milik,apt_stok_unit.kd_pabrik,apt_stok_unit.tgl_expire,apt_stok_unit.harga_pokok,apt_stok_unit.batch');		
		if(!empty($nama_obat))$this->datatables->like('nama_obat',$nama_obat,'both');
	...
	public function indikatorobat(){
		...
		$data=array('items'=>$this->db->join('apt_obat','apt_indikator_obat.kd_obat=apt_obat.kd_obat')->get('apt_indikator_obat')->result_array(),
...
public function tutupbulan(){
	...
	public function tutupbulanpkm(){
		...
		$jsfooter=array();
		$datafooter=array('jsfile'=>$jsfooter);
		$data=array('datapuskesmas'=>$this->mlaporanapt->ambilData('gfk_puskesmas'),
					//'items'=>$this->mlaporanapt->getMutasiObat($kd_unit_apt,$bulan,$tahun),
	...
	public function rl1excelpenjualanobat($periodeawal="",$periodeakhir="",$puskesmas=""){ //bwt yg penjualan obat
		$this->load->library('phpexcel'); 
		$this->load->library('PHPExcel/iofactory');
		$objPHPExcel = new PHPExcel(); 
		$objPHPExcel->getProperties()->setTitle("title")->setDescription("description");
	...
	public function rl1excelrekappenjualan($periodeawal="",$periodeakhir="",$kd_unit_apt="",$is_lunas="",$resep="",$status="",$kd_jenis_bayar="",$cust_code=""){
		$resep1=""; $kd_jenis_bayar1=""; $cust_code1="";
		$this->load->library('phpexcel'); 
		$this->load->library('PHPExcel/iofactory');
		$objPHPExcel = new PHPExcel(); 
		$objPHPExcel->getProperties()->setTitle("title")->setDescription("description");
		...
		if($kd_jenis_bayar=="null"){$jenisbayar='';}
		else{
			$jenis=$this->mlaporanapt->jenisbayar($kd_jenis_bayar);	
			if($cust_code=="null"){$cust_code2='';}
			else{
		...
		//debugvar($status);
		if($resep=='null'){$resep1='';} 
		else {$resep1=$resep;}	
		
		if($kd_jenis_bayar=='null'){$kd_jenis_bayar1='';} 
		else {$kd_jenis_bayar1=$kd_jenis_bayar;}
		
		if($cust_code=='null'){$cust_code1='';} 
		else {$cust_code1=$cust_code;}
		...
		foreach ($items as $item) {
			...
			$nomor=$nomor+1; $baris=$baris+1; 
			$totaljumlah=$totaljumlah+$item['qty']; $totalservice=$totalservice+$item['service']; $totalpenjualan=$totalpenjualan+$item['totalsemua']; 
		}
		for($x='A';$x<='D';$x++){
			if($x=='A'){ 
			...
			$objPHPExcel->getActiveSheet()->getStyle($x.($baris+1))->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,
																					  'size'		=>13,'color'     => array('rgb' => '000000'))));
			$objPHPExcel->getActiveSheet()->getStyle($x.($baris+2))->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,
																					  'size'		=>13,'color'     => array('rgb' => '000000'))));
			$objPHPExcel->getActiveSheet()->getStyle($x.($baris+3))->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,
																					  'size'		=>13,'color'     => array('rgb' => '000000'))));
		}
	...
	public function rl1excelkartustok($kd_obat="",$kd_unit_apt="",$bulan="",$tahun=""){
		...
		foreach ($items as $item) {
			...
			if($item['keterangan']=='Saldo Awal' or $item['keterangan']=='Penerimaan Vendor' or $item['keterangan']=='Pengeluaran Ke Puskesmas' or $item['kode']=='M'){ $objPHPExcel->getActiveSheet()->setCellValue ('D'.$baris,$item['unitvendor']);}
			else{
	...
	public function indikatorobatxls($bulan="",$tahun=""){
        ...
        foreach ($puskesmas as $pkm) {
			...
			$objPHPExcel->getActiveSheet()->getStyle($kolompuskesmas.'10')->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
																			'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,'size'		=>11 /*untuk ukuran tulisan judul kolom2 di tabel*/,'color'     => array('rgb' => '000000')),
																			'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')))));
			$objPHPExcel->getActiveSheet()->getStyle($kolompuskesmas.'11')->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
																			'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,'size'		=>11 /*untuk ukuran tulisan judul kolom2 di tabel*/,'color'     => array('rgb' => '000000')),
																			'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
		...
		foreach ($items as $item) {
            ...
            # code...
				...
				$objPHPExcel->getActiveSheet()->getStyle($kolompuskesmas.$baris)->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
																				'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,'size'		=>11 /*untuk ukuran tulisan judul kolom2 di tabel*/,'color'     => array('rgb' => '000000')),
																				'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																				'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																				'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
	...
	public function rl1excelpersediaanobat($stok="",$isistok="",$kd_unit_apt="",$kd_golongan=""){
		...
		$namaunit=$this->mlaporanapt->namaUnit($kd_unit_apt);
		if($namaunit=='0' or $namaunit=='') {$objPHPExcel->getActiveSheet()->setCellValue ('A5','Unit : Semua Sumber');}
		else {$objPHPExcel->getActiveSheet()->setCellValue ('A5','Unit : '.$namaunit);}
		...
		for($x='A';$x<='K';$x++){
			...
			$objPHPExcel->getActiveSheet()->getStyle($x.'8')->applyFromArray(array('font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
																			'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,'size'		=>11 /*untuk ukuran tulisan judul kolom2 di tabel*/,'color'     => array('rgb' => '000000')),
																			'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
																			'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
		...
		foreach ($items as $item) {
			...
			$nomor=$nomor+1; $baris=$baris+1; $total=$total+$nilai;
		}	
		for($x='A';$x<='K';$x++){
			if($x=='A'){
				$objPHPExcel->getActiveSheet()->getStyle($x.$baris)->getAlignment()->applyFromArray(
	...
	public function logpenerimaan(){
		...
		$data=array('items'=>$this->mlaporanapt->getLog($periodeawal,$periodeakhir,$kd_supplier,$jenis),
					'unitapotek'=>$this->mlaporanapt->ambilData('apt_unit'),
					'periodeawal'=>$periodeawal,
					'periodeakhir'=>$periodeakhir,
					'kd_supplier'=>$kd_supplier,
	...
	public function excelmutasiobat($kd_unit_apt="",$bulan="",$tahun=""){
		...
		foreach ($items as $item) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue ('D'.$baris,number_format($item['in_pbf'],2,'.',',')); 		$pbf=$item['in_pbf'];
			$objPHPExcel->getActiveSheet()->setCellValue ('E'.$baris,number_format($item['in_unit'],2,'.',',')); 		$inunit=$item['in_unit'];
			$objPHPExcel->getActiveSheet()->setCellValue ('F'.$baris,number_format($item['retur_jual'],2,'.',',')); 	$retkeluar=$item['retur_jual'];$jml=$pbf+$inunit+$retkeluar; //$retpbf=$item['retur_pbf'];$jml=($pbf+$inunit)-$retpbf;
			...
			$objPHPExcel->getActiveSheet()->setCellValue ('H'.$baris,number_format($item['out_jual'],2,'.',',')); 	$resep=$item['out_jual'];
			$objPHPExcel->getActiveSheet()->setCellValue ('I'.$baris,number_format($item['out_unit'],2,'.',',')); 	$outunit=$item['out_unit']; $retpbf=$item['retur_pbf'];//$retkeluar=0;
			$objPHPExcel->getActiveSheet()->setCellValue ('J'.$baris,number_format($retpbf,2,'.',',')); 			$jml1=$resep+$outunit+$retpbf;
	...
	public function rl1excelaliranbarang($periodeawal, $periodeakhir, $limit, $kd_unit_apt, $mode) {
		...
		// change
		$title = '';
		if($mode == 1) { $title = 'LAPORAN FAST MOVING'; } elseif($mode == 2) { $title = 'LAPORAN SLOW MOVING'; }
	...
	public function excellplpogolonganbulanan($bulan="",$bulansatu="",$tahun="",$kd_golongan=""){
		...
		foreach ($items as $item) {
			...
			//$total=$item['total_saldo_akhir']*$item['harga_beli'];
                ...
                if($item['total_jum_keluar']!=0)$item['rata_pemakaian']=$item['total_jum_keluar'] / $jum_bulan; else $item['rata_pemakaian']=0;
...
//         ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		for($x='A';$x<='R';$x++){
			...
			$objPHPExcel->getActiveSheet()->getStyle($x.'7')->applyFromArray(array(
					'font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER, 'bold' => true,
					'size'		=>11,'color'     => array('rgb' => '000000')),'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
					'color' => array('rgb' => '000000')),'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
					'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
					'color' => array('rgb' => '000000')))));
			...
			$objPHPExcel->getActiveSheet()->getStyle($x.'9')->applyFromArray(array(
					'font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER, 'bold' => false,
					'size'		=>11,'color'     => array('rgb' => '000000')),'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
					'color' => array('rgb' => '000000')),'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
					'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
					'color' => array('rgb' => '000000')))));
		...
		foreach ($sumberdana as $sd) {
			for($x='A';$x<='R';$x++){
				$objPHPExcel->getActiveSheet()->getStyle($x.$baris)->applyFromArray(array(
						'font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER, 'bold' => true,
						'size'		=>11,'color'     => array('rgb' => '000000')),'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
						'color' => array('rgb' => '000000')),'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
						'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
						'color' => array('rgb' => '000000')))));
			...
			$objPHPExcel->getActiveSheet()->setCellValue ('A'.$baris,$array_romawi[$no_sd]);$no_sd++;
			...
			foreach ($items as $item) {
				...
				for($x='A';$x<='C';$x++){
					...
					$objPHPExcel->getActiveSheet()->getStyle($x.$baris)->applyFromArray(array(
							'font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER, 'bold' => false,
							'size'		=>11,'color'     => array('rgb' => '000000')),'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
							'color' => array('rgb' => '000000')),'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
							'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
							'color' => array('rgb' => '000000')))));
			...
			for($x='A';$x<='R';$x++){
				$objPHPExcel->getActiveSheet()->getStyle($x.$baris)->applyFromArray(array(
						'font'    => array('name'      => 'calibri','horizontal' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER, 'bold' => true,
						'size'		=>11,'color'     => array('rgb' => '000000')),'borders' => array('bottom'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
						'color' => array('rgb' => '000000')),'top'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),
						'left'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => '000000')),'right'     => array('style' => PHPExcel_Style_Border::BORDER_THIN,
						'color' => array('rgb' => '000000')))));
...
');
	...
	public function cetakobatsas(){
		$this->load->library('phpexcel'); 
		$this->load->library('PHPExcel/iofactory');
		//$objPHPExcel = new PHPExcel(); 
		$objPHPExcel = IOFactory::load("./template/lapobatsas.xls");
		$objPHPExcel->getProperties()->setTitle("title")->setDescription("description");
		$items=array();
		$items=$this->db->query('select *,apt_stok_unit.* from apt_stok_unit 
					                join apt_obat on apt_stok_unit.kd_obat=apt_obat.kd_obat 
		...
		foreach ($items as $item) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue ('A'.$baris,$nomor);
			$objPHPExcel->getActiveSheet()->setCellValue ('B'.$baris,$item['nama_obat']);
			$objPHPExcel->getActiveSheet()->setCellValue ('C'.$baris,$item['kd_satuan_kecil']); 	
			$objPHPExcel->getActiveSheet()->setCellValue ('E'.$baris,$item['batch']); 	
			$objPHPExcel->getActiveSheet()->setCellValue ('F'.$baris,convertDate($item['tgl_expire'])); 	
			$objPHPExcel->getActiveSheet()->setCellValue ('G'.$baris,$item['nama_pabrik']); 	
			$objPHPExcel->getActiveSheet()->setCellValue ('H'.$baris,$item['kode_sas']); 	
			$objPHPExcel->getActiveSheet()->setCellValue ('I'.$baris,$item['nama_unit_apt']); 	
            ...
            if(!empty($so)){
			    $objPHPExcel->getActiveSheet()->setCellValue ('J'.$baris,number_format($so['stok_baru'],2,'.',',')); 
            }

            $penerimaan = $this->db->select('*,date(tgl_penerimaan) as tgl_penerimaan')
            ...
            if(!empty($penerimaan)){
                foreach ($penerimaan as $pen){
    			    ...
    			    $objPHPExcel->getActiveSheet()->setCellValue ('K'.$baris,convertDate($pen['tgl_penerimaan'])); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('L'.$baris,$pen['no_faktur']); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('M'.$baris,$pen['nama']); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('N'.$baris,$pen['qty_kcl']); 
        			$baris=$baris+1; 
                }
            }
            ...
            if(!empty($pengeluaran)){
                foreach ($pengeluaran as $pen){
    			    ...
    			    $objPHPExcel->getActiveSheet()->setCellValue ('O'.$baris,convertDate($pen['tgl_penjualan'])); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('P'.$baris,$pen['no_sbbk']); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('Q'.$baris,$pen['nama']); 
    			    $objPHPExcel->getActiveSheet()->setCellValue ('R'.$baris,$pen['qty']); 
        			$baris=$baris+1; 
                }
            }
		...
		$objWriter = IOFactory::createWriter($objPHPExcel, 'Excel5'); 
		$objWriter->save("download/lapobatsas.xls");
