...
class Penjualan extends Rumahsakit
{
	...
	public function simpanpenjualan()
	{
		...
		if ($this->mpenjualan->isNumberExist($no_penjualan)) { //edit
			...
			$this->db->trans_start();
			if ($tgl_penjualan == '') {
				$tgl_penjualan = convertDate($tglpenjualan);
			}
	...
	public function ambildaftarobatbynama()
	{
		...
		$this->datatables->select('apt_stok_unit.kd_obat, replace(apt_obat.nama_obat,"\'","") as nama_obat, apt_satuan_kecil.satuan_kecil, date_format(apt_stok_unit.tgl_expire,"%d-%m-%Y") as tgl_expire,apt_stok_unit.batch,apt_stok_unit.kode_sas,nama_pabrik,
	...
	public function cetakSbbkExcel($no_penjualan)
	{
		...
		$sheet->getColumnDimension('K')->setWidth(7.5); // no batch
		...
		$sheet->setCellValue('K9', 'KODE SAS');
		...
		foreach ($items as $item) {
			...
			$sheet->setCellValue('K' . $rowCellNum, $item['kode_sas']);         // col 7: keterangan/ tgl_keluar
	...
	{
		$output = shell_exec('docker run -it -v /www/wwwroot/simo.balikpapan.go.id/ifk/belajar-composer:/usr/src/myapp -w /usr/src/myapp php php mail.php');
		print_r($output);
	}
}
