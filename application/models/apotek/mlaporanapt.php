...
class Mlaporanapt extends CI_Model
{
	...
	function getAllPenerimaanApotek($periodeawal, $periodeakhir, $kd_unit_apt, $kd_supplier)
	{
		...
		if ($kd_unit_apt != '') {
			$kd_unit_apt = " and apt_penerimaan.kd_unit_apt='" . $kd_unit_apt . "'";
		} else {
			$kd_unit_apt = '';
		}

		if ($kd_supplier != '') {
			$kd_supplier = " and apt_supplier.kd_supplier='" . $kd_supplier . "'";
		} else {
			$kd_supplier = '';
		}
	...
	function getAllPenjualanApotek($periodeawal, $periodeakhir, $shiftapt, $is_lunas, $resep)
	{
		...
		if ($resep != '') {
			$b1 = " and apt_penjualan.resep='$resep'";
		} else {
			$b1 = "";
		}
		$b = $b1;
	...
	function getKartuStokBatch($kd_obat, $kd_unit_apt, $kd_pabrik, $tgl_expire, $harga_pokok, $batch)
	{
	...
	function getMutasiObatTriwulan($tahun, $triwulan, $kd_kategori = '')
	{
		if ($triwulan == '1') {
			$bulan_awal = '01';
			$bulan_akhir = '03';
		...
		if ($kd_kategori != '') {
			$kat = ' and apt_obat.kd_kategori = "' . $kd_kategori . '" ';
		} else {
			$kat = '';
		}
	...
	function getMutasiObat($kd_unit_apt, $bulan, $tahun, $kd_kategori = '')
	{
		if ($kd_kategori != '') {
			$kat = ' and apt_obat.kd_kategori = "' . $kd_kategori . '" ';
		} else {
		...
		if (empty($kd_unit_apt)) {
			$query = $this->db->query("select apt_satuan_kecil.satuan_kecil,apt_mutasi_obat.kd_obat,apt_obat.nama_obat,ifnull(sum(apt_mutasi_obat.saldo_awal),0) as saldo_awal,ifnull(sum(apt_mutasi_obat.in_pbf),0) as in_pbf,
					...
					apt_mutasi_obat.bulan='$bulan' and apt_mutasi_obat.tahun='$tahun' $kat
					group by apt_mutasi_obat.kd_obat,apt_mutasi_obat.harga_beli
					having saldo_awal > 0 or in_pbf > 0 or out_jual > 0 or out_disposal > 0 or saldo_akhir >0  or stok_opname > 0
					order by apt_mutasi_obat.kd_obat");
		} else {
			$query = $this->db->query("select apt_satuan_kecil.satuan_kecil,apt_mutasi_obat.kd_obat,apt_obat.nama_obat,ifnull(sum(apt_mutasi_obat.saldo_awal),0) as saldo_awal,ifnull(sum(apt_mutasi_obat.in_pbf),0) as in_pbf,
					...
					apt_mutasi_obat.bulan='$bulan' and apt_mutasi_obat.tahun='$tahun' $kat group by apt_mutasi_obat.kd_obat,apt_mutasi_obat.harga_beli
					having saldo_awal > 0 or in_pbf > 0 or out_jual > 0 or out_disposal > 0 or saldo_akhir >0  or stok_opname > 0
					order by apt_mutasi_obat.kd_obat");
		}
	...
	function getMutasiObatBulanan($kd_unit_apt, $bulan, $bulan1, $tahun, $kd_nomenklatur = '', $kategori = '', $kd_kategori = '')
	{
		...
		if ($kd_kategori != '') {
			$kat2 = ' and apt_obat.kd_kategori = "' . $kd_kategori . '" ';
		} else {
			$kat2 = '';
		}
		...
		if (empty($kd_unit_apt)) {
			...
			$query = $this->db->query("select apt_satuan_kecil.satuan_kecil,apt_mutasi_obat.kd_obat,apt_obat.nama_obat,ifnull(sum(apt_mutasi_obat.in_pbf),0) as in_pbf,ifnull(sum(apt_mutasi_obat.in_unit),0) as in_unit,ifnull(sum(apt_mutasi_obat.retur_jual),0) as retur_jual,
					...
					where apt_obat.kd_satuan_kecil=apt_satuan_kecil.kd_satuan_kecil and apt_mutasi_obat.kd_obat=apt_obat.kd_obat and
					apt_mutasi_obat.bulan>='$bulan' and apt_mutasi_obat.bulan<='$bulan1' and apt_mutasi_obat.tahun='$tahun' $nomen $kat $kat2
	...
	{
		//$kd_unit_apt=$this->session->userdata('kd_unit_apt');
		if (empty($harga_pokok)) $harga = "";
		else $harga = " and round(apt_penerimaan_detail.harga_pokok)='" . $harga_pokok . "'";
	...
	function getPenerimaanDistribusiObat($obat, $harga_pokok = "", $bulan, $tahun, $kd_unit_apt)
	{
		...
		if (empty($harga_pokok)) $harga = "";
		else $harga = " and apt_penerimaan_detail.harga_pokok='" . $harga_pokok . "'";
		$query = $this->db->query("select apt_obat.kd_obat,apt_obat.nama_obat,apt_distribusi.tgl_distribusi,ifnull(sum(apt_distribusi_detail.qty),0) as qty from apt_obat,apt_distribusi,apt_distribusi_detail,
				apt_unit where apt_obat.kd_obat=apt_distribusi_detail.kd_obat
				and apt_distribusi.no_distribusi=apt_distribusi_detail.no_distribusi and apt_unit.kd_unit_apt=apt_distribusi.kd_unit_tujuan
	...
	function getPenjualanObat($kd_obat, $harga_pokok = "", $bulan, $tahun, $kd_unit_apt)
	{
		...
		if (empty($harga_pokok)) $harga = "";
		else $harga = " and round(apt_penjualan_detail.harga_pokok)='" . $harga_pokok . "'";
	...
	function getDisposalObat($kd_obat, $harga_pokok = "", $bulan, $tahun, $kd_unit_apt)
	{
		...
		if (empty($harga_pokok)) $harga = "";
		else $harga = " and round(apt_disposal_detail.harga_pokok)='" . $harga_pokok . "'";
	...
	function stokopnameObat($obat, $harga_pokok = "", $bulan, $tahun, $kd_unit_apt)
	{
		...
		if (empty($harga_pokok)) $harga = "";
		else $harga = " and round(history_perubahan_stok.harga)='" . $harga_pokok . "'";
	...
	function QueryPenjualanObat($periodeawal, $periodeakhir, $kd_unit_apt, $kd_obat)
	{
		...
		if ($kd_unit_apt != '') {
			$a1 = " and apt_penjualan.kd_unit_apt='$kd_unit_apt'";
		} else {
			$a1 = "";
		}
		$a = $a1;
		...
		if ($kd_obat != '') {
			$b1 = " and apt_penjualan_detail.kd_obat='$kd_obat'";
		} else {
			$b1 = "";
		}
		$b = $b1;
	...
	function ambilIsiPenjualan($periodeawal, $periodeakhir, $kd_unit_apt)
	{
		...
		if ($kd_unit_apt != '') {
			$kd_unit_apt = " and apt_penjualan_detail.kd_unit_apt='$kd_unit_apt'";
		} else {
			$kd_unit_apt = "";
		}
	...
	function getLog($periodeawal, $periodeakhir, $kd_supplier, $jenis)
	{
		...
		if ($kd_supplier != '') {
			$a1 = "and apt_penerimaan.kd_supplier='$kd_supplier'";
		} else {
			$a1 = "";
		}
		$a = $a1;
		...
		if ($jenis != '') {
			$b1 = "and apt_log_penerimaan.jenis='$jenis'";
		} else {
			$b1 = "";
		}
		$b = $b1;
	...
	{
		if (!empty($status)) $stat = " and p.status='$status'";
		else $stat = "";
		$query = $this->db->query(
			"SELECT o.*,pd.kd_obat, o.nama_obat, sk.satuan_kecil,unit.nama_unit_apt,pd.kd_unit_apt,pd.keterangan, pd.qty as jumlah
	...
	{
		$query = $this->db->query(
			"SELECT pd.kd_obat, o.nama_obat, sk.satuan_kecil, SUM(pd.qty) as jumlah
